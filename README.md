...
## 노션 본문 작성 규칙 (홈 페이지 — 레이어가 이 구조를 읽는다)
```
[html 코드블록 = oopy/loader.html]
# 📃 NOTICE
### 공지사항          ← 그룹명이 카드 태그가 된다
...
### 자료실
- @페이지멘션
### 🔗 Quick links      ← 칩 링크. "라벨 : [바로가기](url)" 형식
### 회사소개            ← 하위 페이지(인사말·오시는 길·연락하기) = 푸터·문의 섹션 링크
[oopy:hide 토글: 내부 안내·숨김 페이지]
```
- 🔴 **탭 블록·컬럼 안에 공지를 넣지 말 것** — 활성 탭만 렌더돼 레이어가 못 읽는다. 위처럼 평면 목록으로.
- 방문자에게 노션 본문은 보이지 않는다(레이어가 데이터로만 읽고 숨김). 레이어가 못 뜨면(GitHub 장애) 노션 본문이 그대로 보이는 것이 안전망.

## 운영
- **공지 추가·수정** → 노션 홈의 NOTICE 목록에 한 줄 추가/수정 → **즉시 반영**(레이어가 페이지 DOM에서 읽음). `data/notices.json`은 단독 미리보기·대체용(매시 동기).
